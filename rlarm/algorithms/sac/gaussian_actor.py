...
from rlarm.algorithms.sac.distributions.diagonal_gaussian import DiagonalGaussian


# From TF2RL Github Repo
class GaussianActor(tf.keras.Model):
    ...
    def __init__(self, state_shape, action_dim, max_action,
        ...
        self.l1 = tf.keras.layers.Dense(units[0], name="L1", activation=hidden_activation)
        self.l2 = tf.keras.layers.Dense(units[1], name="L2", activation=hidden_activation)
        self.out_mean = tf.keras.layers.Dense(action_dim, name="L_mean")
        if not self._fix_std:
            if self._state_independent_std:
                ...
                self.out_log_std = tf.keras.layers.Dense(
